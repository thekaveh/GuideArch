...
| search capabilities | yes (`ISearchable`, `SearchableState`) | yes (`ISearchable`, `SearchableState<T>`) | yes (`ISearchable`, `SearchableState<TItem>`) | medium | Similar to filter capabilities: available everywhere and plausible for future UX, but not a top migration target today. `SearchableState` / `SearchableState<T>` / `SearchableState<TItem>` is the clearest off-the-shelf helper when GuideArch wants more than the bare interface. |
