...
The M2 skeleton lets a user OPEN a scenario and SEE the resulting ranked candidates. M3 lets them EDIT scenarios in-place. Editing any field that the spec §3.3 (in `viewmodels.md`) marks "triggers solve" must re-run TOPSIS with a 100 ms debounce and update the candidates table live.
